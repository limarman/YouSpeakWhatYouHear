...
def align_multiple_cmd(
	...
	print_matrix: bool = typer.Option(True, help="Print similarity matrix and component submatrix"),
) -> None:
	...
	# Print full matrix if requested
	if print_matrix:
		from rich.table import Table as _Table
		headers = [os.path.basename(f) for f in files]
		tbl = _Table(title="Pairwise similarity (combined score)")
		tbl.add_column("")
		for h in headers:
			tbl.add_column(h)
		for i in range(len(files)):
			row_vals = [f"{float(v):.3f}" for v in similarity_matrix[i]]
			tbl.add_row(headers[i], *row_vals)
		print(tbl)
	...
	for idx in component_indices:
		print(f"  • {files[idx]}")
	# Print submatrix for component if requested
	if print_matrix and len(component_indices) > 0:
		from rich.table import Table as _Table
		comp_headers = [os.path.basename(files[i]) for i in component_indices]
		tblc = _Table(title=f"Component submatrix (threshold={component_threshold})")
		tblc.add_column("")
		for h in comp_headers:
			tblc.add_column(h)
		for ri, i in enumerate(component_indices):
			row = [f"{float(similarity_matrix[i][j]):.3f}" for j in component_indices]
			tblc.add_row(comp_headers[ri], *row)
		print(tblc)
...
# =============================================================================
# NEW ALIGNMENT MODULE COMMANDS
# =============================================================================

@app.command(name="align-pair-new")
def align_pair_new_cmd(
	file_a: str = typer.Argument(..., help="First subtitle file (.srt/.vtt)"),
	file_b: str = typer.Argument(..., help="Second subtitle file (.srt/.vtt)"),
	n: int = typer.Option(3, help="Character n-gram size for similarity"),
	gap_penalty: float = typer.Option(-0.4, help="Gap penalty for NW alignment"),
	...
	from .analysis.alignment_new import align_subtitle_pair, BlockAlignmentConfig
	...
	# Import internal NW function to get raw alignment
	from .analysis.alignment_new import _needleman_wunsch_align, _compute_blocks_growmerge
	...
	from .util.types import BlockAlignment
	blocks_file_a = []
	blocks_file_b = []
	similarity_scores = []
...
@app.command(name="align-pipeline-new")
def align_pipeline_new_cmd(
	files: list[str] | None = typer.Argument(None, help="Subtitle files (.srt/.vtt) to align"),
	dir: str | None = typer.Option(None, "--dir", help="Folder containing candidate .srt/.vtt files"),
	recursive: bool = typer.Option(False, help="Recursively search for .srt/.vtt under --dir"),
	n: int = typer.Option(3, help="Character n-gram size for similarity"),
